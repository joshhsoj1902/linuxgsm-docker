...
for queryattempt in {1..5}; do
	...
	if [ "$(cat "${rootdir}/${lockselfname}")" -gt "$(date "+%s" -d "${querydelay} mins ago")" ]; then
		...
		"${functionsdir}"/query_gsquery.py -a "${query_ip}" -p "${queryport}" -e "${engine}" > /dev/null 2>&1
		querystatus="$?"
	elif [ "${querymethod}" ==  "tcp" ]; then
		bash -c 'exec 3<> /dev/tcp/'${query_ip}'/'${queryport}''
		querystatus="$?"
	elif [ "${querymethod}" ==  "udp" ]; then
		bash -c 'exec 3<> /dev/udp/'${query_ip}'/'${queryport}''
		querystatus="$?"
	fi
	...
	if [ "${querystatus}" == "0" ]; then
		...
		# Server query FAIL.
		fn_script_log_info "Querying port: ${querymethod}: ${query_ip}:${queryport} : ${queryattempt}: FAIL"
		...
		if [ "${querymethod}" ==  "gsquery" ]; then
			if [ "${totalseconds}" -ge "59" ]; then
				...
				fn_script_log_error "Querying port: ${querymethod}: ${query_ip}:${queryport} : ${queryattempt}: FAIL"
		...
		for seconds in {1..15}; do
			fn_print_fail "Querying port: ${querymethod}: ${query_ip}:${queryport} : ${totalseconds}/${queryattempt}: WAIT"
